...
# What a price monitor on Rakuten actually needs to watch, which is more
# than the price.
#
# `points` and `point_rate` are in here and that is the Rakuten-specific
...
# (`"itemdata"`) publishes one where Rakuten's own verification flag allows
...
def _within_tolerance(before: dict, after: dict, changes: dict,
    ...
    RAKUTEN BEHAVIOUR WAS MEASURED, and it makes this flag doubly
...
def diff_products(old: List[dict], new: List[dict],
    ...
    # a placement move would otherwise read as a price change. Rakuten also
...
def _check_comparable(args) -> bool:
    ...
    # hostname alone. Rakuten Ichiba is ONE marketplace with ONE currency —
    ...
    # routes, measured 2026-09-21 — so `source` is "rakuten.co.jp" on both
    ...
    if len(set(currencies.values())) > 1:
        problems.append(
            ...
            f"Rakuten Ichiba prices everything in JPY for every visitor — "
            ...
            f"it is 'rakuten.co.jp' on both sides.")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Diff two rakuten-scraper JSON outputs by sku.")
    ...
    p.add_argument("--price-tolerance-pct", type=float, default=0.0,
                   ...
                   help="Treat a price move smaller than PCT%% as an exchange-"
                        ...
                        "0, which is what a Rakuten run wants for two "
