...
def block_advice(html: Optional[str], headless: bool,
    ...
    if marker == "akamai-reference-id":
        return (
            ...
            "User-Agent. So check for a fingerprint CONTRADICTION before "
            "buying anything: do not set --fingerprint or a custom UA over "
            "--cdp-endpoint (the remote browser brings its own), and do not "
            "put a browser UA on an HTTP client. If the client is already "
            "consistent, %s"
            % (marker,
