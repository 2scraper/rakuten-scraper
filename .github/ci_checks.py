...
#
# Rakuten publishes a 32-hex identifier of its own — the cross-merchant
# catalogue id in `productUrl`
# (`product.rakuten.co.jp/product/-/55b617dc…/`) — so this repo had the same
...
# arrangement: an allowlist forgiving 32-hex inside a rakuten.co.jp URL is a
...
# repo does not need the exemption: `make_fixtures.py` replaces Rakuten's own
...
CAPTURES_IN_HISTORY_DECIDED = {
    ...
    # sponsored slot. All of it is Rakuten's own rather than ours — no
